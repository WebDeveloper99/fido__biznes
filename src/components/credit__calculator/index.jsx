...
const Calculator = () => {
  ...
  const getListOfPayement = (
    ...
    tatalCreditAbount = parseFloat(tatalCreditAbount - tatalCreditAbount * (intialPaymentPercentage / 100) ) * 1000000;
    ...
    while (--duration >= 0) {
      if (paymentType === "anuited" || paymentType === undefined) {
        let monthlyAmount =
          tatalCreditAbount * (loanPercentage / (duration * 100) + (loanPercentage / (duration * 100)) / (Math.pow(1 + loanPercentage / (duration * 100), duration) - 1));
